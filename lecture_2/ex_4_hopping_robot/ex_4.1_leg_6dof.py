...
""" It is a template of Pranav Bhounsule
    ...
    (1) Joint references are given
    (2) Torso has 6 DOF

 """

xml_file_name = '6dof_hopping_robot_.xml'
...
def pos_controller(model, data, ref, i):
    ...
    set_position_servo(1, 90)
    set_velocity_servo(2, 2)
    data.ctrl[1] = ref[0][i]

    set_position_servo(4, 90)
    set_velocity_servo(5, 2)
    data.ctrl[4] = ref[1][i]

    set_position_servo(7, 90)
    set_velocity_servo(8, 2)
    data.ctrl[7] = ref[2][i]

    set_position_servo(10, 90)
    set_velocity_servo(11, 2)
    data.ctrl[10] = ref[3][i]
...
frq = 6.5e1
ref = [np.pi/6 * np.sin(frq * tt) + np.pi/4,
       -np.pi/3 * np.sin(frq * tt) - np.pi/2,
       np.pi/6 * np.sin(frq * tt) + np.pi/4,
       -np.pi/3 * np.sin(frq * tt) - np.pi/2]
...
while not glfw.window_should_close(window):
    ...
    # print(f"Theta_A={data.qpos[1]:.2f} and Theta_B={data.qpos[2]:.2f}")
    # print(f"Theta_C={data.qpos[3]:.2f} and Theta_D={data.qpos[4]:.2f}")
